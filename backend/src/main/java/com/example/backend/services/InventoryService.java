...
public class InventoryService {
    ...
    public Inventory update(Long id, InventoryUpdateDTO dto, User currentUser) {
        ...
        Product updateProduct = inventory.getProduct().getId().equals(
                Optional.ofNullable(dto.getProductId()).orElse(inventory.getProduct().getId())) ? inventory.getProduct()
        ...
        Integer updatePackageValue = Optional.ofNullable(dto.getPackageValue()).orElse(inventory.getPackageValue());
        Zone updateZone = inventory.getZone().getId().equals(
                Optional.ofNullable(dto.getZoneId()).orElse(inventory.getZone().getId())) ? inventory.getZone()
                        : zoneRepository.findByIdAndWarehouse_ShopId(dto.getZoneId(), dto.getZoneId())
                                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy kho hàng!"));
